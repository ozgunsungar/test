...
namespace KnBalloonTool.Services
{
    public static class ExcelExportService
    {
        public static void Export(string path, IEnumerable<KnBalloonRecord> records)
        {
            ...
            {
                var ws = wb.Worksheets.Add("KN Listesi");

                ws.Cell(1, 1).Value = "KN No";
                ws.Cell(1, 2).Value = "Kaynak";
                ws.Cell(1, 3).Value = "Anlık Değer";
                ws.Cell(1, 4).Value = "Üst Tol.";
                ws.Cell(1, 5).Value = "Alt Tol.";
                ws.Cell(1, 6).Value = "Lokasyon";
                ws.Cell(1, 7).Value = "Journal ID";
                ws.Cell(1, 8).Value = "Not";
                ...
                foreach (var r in records)
                {
                    ...
                    ws.Cell(row, 2).Value = r.SourceLabel ?? string.Empty;
                    ...
                    ws.Cell(row, 6).Value = r.Location ?? string.Empty;
                    ws.Cell(row, 7).Value = r.JournalId ?? string.Empty;
                    ws.Cell(row, 8).Value = r.Note ?? string.Empty;
                    row++;
                }
